...
# adapted from the LFPy Cell class.
"""
...
import pickle
from .run_simulation import _run_simulation, \
     _run_simulation_with_electrode, _collect_geometry_neuron

import linear_alg_routines as linar #v
import time #v #debug
import datetime #v #debug
from tms_networks.debugging import * #v
...
##                    delete_sections=True,
        ...
        self.pt3d = pt3d
        self.cell_seclist = cell_seclist #v
...
##        if delete_sections:
##            print "--delete_sections: ", delete_sections
##            numsec = 0
...
##            neuron.h('forall delete_section()')
        ...
        numsec = 0
        for numsec, sec in enumerate(cell_seclist):
            pass
        if numsec > 0 and self.verbose:
            mssg = "%s sections detected! " % numsec
            warn(mssg)
	t_end = time.time(); write_log('time', \
           [ 'lfpy checks (h:min:s): ', \
             t_end, t_start]) #debug
...
##        # set some additional attributes (v: needs sectionlist in 'self.morphology')
##        self._create_sectionlists_from_morphology_value()

        #v: there is no .rot file, so it starts empty
        self.default_rotation = {}
	#t_end = time.time(); write_log('time', ['morph init', t_end, t_start])
	...
	t_start = time.time(); #debug 
        ...
        # v: after instantiating one of the Aberra et al 2018 cells and running the '_set_nsegs' line,
        # calling 'h.finitialize' will give a segmentation fault.
##        self._set_nsegs(nsegs_method, lambda_f, d_lambda, max_nsegs_length)
	...
	#t_end = time.time(); write_log('time', ['xcell, nsegs', t_end-t_start])
	...
	#t_end = time.time(); write_log('time', ['extract pt3d', t_end, t_start])
	...
	#t_end = time.time(); write_log('time', ['update/collect 3d', t_end, t_start])
	
	t_start = time.time(); #debug
        if hasattr(self, 'somapos'):
            #print "somapos: ", self.somapos #v
            self.set_pos()
        else:
            if self.verbose:
                print('no soma, using the midpoint if initial segment.')
#        self.set_rotation(**self.default_rotation)
	#t_end = time.time(); write_log('time', ['set somapos and rot', t_end, t_start])
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
    def _create_sectionlists(self):
        ...
        for sec in self.cell_seclist: #v
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
    def simulate(self, electrode=None, rec_imem=False, rec_vmem=False,
        ...
        if rec_vmem:
            self._collect_vmem()
        #self._collect_isyn() #synapses
        #self._collect_vsyn() #synapses
        #self._collect_istim()
        #self._collect_vstim()
        if len(rec_variables) > 0:
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
	...
	t_start = time.time(); #debug
        ...
        #self._update_synapse_positions() #syn
# ---------------------------------------------------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
    def set_rotation_vectorial(self, initial_vector, final_vector): #v
        """
        Rotate geometry of cell object according to the direction given by
...
##        rel_start = np.dot(M_rot, rel_start.T)
##        rel_end = np.dot(M_rot, rel_end.T)
...
##            if v_ext.shape[0] != self.totalnseg: #v
#                raise ValueError("v_ext.shape[0] != cell.totnsegs")
            ...
            import pdb; pdb.set_trace() #v #remov #debug
        ...
        i = 0
#        for sec in self.allseclist:
        for sec in self.section_list: #v
            for seg in sec:
